...
export default function Menu() {
  ...
  // Фильтруем только валидные пункты меню
  ...
  return (
    <>
      ...
      >
        <span className={`bg-white h-0.5 w-6 rounded transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-0.5' : '-translate-y-1'}`} />
        <span className={`bg-white h-0.5 w-6 rounded transition-all duration-300 ${isOpen ? 'opacity-0' : 'opacity-100'}`} />
        <span className={`bg-white h-0.5 w-6 rounded transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-0.5' : 'translate-y-1'}`} />
      </button>
